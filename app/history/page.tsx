...
export default function Booking() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/booking/historyoff`, {
                    headers: {
                ...
                {car && car.length > 0 ? (
                    car.map((car: any) => (
                        <div className="mt-10 justify-center text-center">
                            <div className="text-[20px] w-[610px] h-[56px] bg-[#E3E3E3] border border-gray-400 rounded-md text-white font-semibold">
                                <p className="mt-[10px] text-[#908F8F]">{formatDate(car.booking_date, car.end_date)}</p>
                            </div>
